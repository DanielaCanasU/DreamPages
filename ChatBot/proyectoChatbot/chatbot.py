...
def chatbot():
    print("¡Hola! Soy un chatbot especializado en libros y personajes. ¿Con qué personaje te gustaría hablar? Solo menciona el nombre.")
    ...
    while True:
        ...
        if personaje_actual is None:
            nombre_personaje, libro = extraer_personaje_y_libro(user_input)
            personaje = buscar_personaje_por_libro(nombre_personaje, libro)
            
            if personaje is not None:
                personaje_actual = personaje
                print(f"Chatbot: ¡Excelente elección! Ahora estás hablando con {personaje['Nombre del Personaje']} ({personaje['Libro']}).")
                continue
            
            libro = buscar_libro(user_input)
            if libro is not None:
                print(f"Chatbot: {libro['Titulo']} ({libro['Genero']}) por {libro['Autor']}. ¿Quieres hablar con algún personaje de este libro?")
                continue
            
            print("Chatbot: No encuentro esa referencia. ¿Podrías reformular o especificar mejor?")
            continue
        
        # Si ya se seleccionó un personaje, se mantiene el contexto de la conversación.
        try:
            respuesta = generar_respuesta(personaje_actual, user_input, contexto_conversacion)
            print(f"\nChatbot (como {personaje_actual['Nombre del Personaje']}): {respuesta}\n")
        except Exception as e:
            print(f"Chatbot (como {personaje_actual['Nombre del Personaje']}): Lo siento, no puedo procesar esa pregunta en este momento.")
            continue
        
        # Actualizar el contexto en orden cronológico: el mensaje del usuario y luego la respuesta.
