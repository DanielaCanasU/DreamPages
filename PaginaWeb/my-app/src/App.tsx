...
import { BookIcon, Search, X, Send, MessageSquare } from 'lucide-react';
...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        // Llamada a la API Flask para obtener recomendaciones
        ...
        const bookRecommendations = data.recommended_books.map((book: { title: string, description: string, author: string, genres: string, pages: string, publishDate: string, cover_image_uri: string, title_es: string, description_es:string }, index: number) => ({
          ...
          author: book.author, // Información de ejemplo ya que la API solo devuelve títulos
...
useEffect(() => {
  ...
  const Tabs = () => (
    <div className="flex justify-center mb-8">
      ...
      <button
        className={`px-4 py-2 ${activeTab === 'title' ? 'bg-primary-100 text-white' : 'bg-white text-primary-100'} rounded-r-lg`}
                    ...
                    >
                      {expandedBookId === book.id ? (
                        <div className="p-8 relative">
                          <div className="flex flex-col lg:flex-row gap-8">
                            ...
                            <div className="lg:w-2/3">
                              <div className="flex justify-between items-start mb-6">
                                <div>
                                  <h3 className="text-2xl font-bold text-primary-100 mb-1">{book.title_es}</h3> {/* Cambiado a book.title_es */}
                                  <p className="text-text-200 text-lg">{book.author}</p>
                                </div>
                              ...
                              <div className="mb-6">
                                <p className="text-text-100 mb-6 leading-relaxed">{book.description_es}</p> {/* Cambiado a book.description_es */}
                          ...
                          {/* Chatbot */}
                          {showChat && (
                            <div className="fixed bottom-6 right-6 w-80 md:w-96 h-[500px] bg-white rounded-2xl shadow-lg border border-bg-200 flex flex-col z-10 overflow-hidden">
